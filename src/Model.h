...
class LinearLayer {
 public:
  ...
  LinearLayer(Matrix& a_, Vector& b_) {
    a = a_;
    b = b_;
  }
...
class Sequential {
 public:
  Sequential(std::initializer_list<size_t> dimensions,
    ...
    assert(linear_layers.back().b.size() == DataLoader::COUNT_OF_DIGITS);
 ...
 public:
  ...
  std::pair<size_t, size_t> Predict(DataLoader &data_loader) {
    ...
    while (!batch.empty()) {
      Conversion(batch);
      for (auto &x_y : batch) {
        a[DataLoader::ConvertVector(x_y.second)][DataLoader::ConvertVector(x_y.first)]++;
        if (DataLoader::ConvertVector(x_y.second) == DataLoader::ConvertVector(x_y.first)) {
          ++count_right_answers;
        }
        b[DataLoader::ConvertVector(x_y.second)]++;
      }
  ...
  friend std::ostream &operator<<(std::ostream &os, const Model &model) {
    os << "Model: " << model.sequential.number_of_layers * 2 << " layers MSE loss " << model.learning_rate << " lr\n";
    os << model.sequential << "\n";
    return os;
  }
  ...
  friend std::istream &operator>>(std::istream &is, Model &model) {
    ...
    size_t cnt_layer;
    std::string name_model, loss_func, loss_func_name, learning_rate;
    is >> name_model >> cnt_layer >> name_model >> loss_func_name >> loss_func >> learning_rate_vol >> learning_rate;
    cnt_layer /= 2;
